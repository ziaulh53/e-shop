<template>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        <router-link v-for="product of data?.products" :to="'/product/' + product?._id" :key="product?._id">
            <div
                class="relative border border-gray-400 rounded-lg bg-white shadow-lg overflow-hidden group hover:scale-105 transition-all duration-300">
                ...
                <div v-if="product?.discountAvailable" class="absolute top-2 right-2 text-sm bg-red-500 text-white font-semibold px-2 py-1">
                   Discount
                </div>
