<template>
    <div class="p-10">
        <div class="flex justify-between border-b pb-8">
            ...
            <h2 class="font-semibold text-2xl">{{ allCart?.length }} Item(s)</h2>
        </div>
        ...
        <div v-for="(cart, idx) of allCart" :key="idx" class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
            ...
            <div class="flex justify-center w-1/5">
                <button @click="()=>cartQuantityDecrease(cart)"><span class="text-gray-600"><i class="fas fa-minus"></i></span></button>

                <input class="mx-2 border text-center w-8" type="text" :value="cart?.quantity">

                <button @click="()=>cartQuantityIncrease(cart)"><span class="text-gray-600"><i class="fas fa-plus"></i></span></button>
            </div>

            <span class="text-center w-1/5 font-semibold text-sm">USD {{cart?.price*cart?.quantity  }}</span>
        </div>
...
const { shoppingCart, cartQuantityIncrease, cartQuantityDecrease } = useCartStore();
const allCart = computed(()=>shoppingCart)
